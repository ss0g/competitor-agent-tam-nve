...
```ts
   55 |   // Try to proceed to next step or find create button
   56 |   try {
   57 |     await expect(nextButton).toBeEnabled({ timeout: 10000 });
   58 |     await nextButton.click();
   59 |     
   60 |     // Step 2: Product Information - Fill required fields
